   async function verifyToken() {
        ...
        try {
          ...
          // Se o token for inválido ou expirado → remove e redireciona
          if (!result.valid) {
            localStorage.removeItem('authToken');
            window.location.href = 'index.html';
          } else {
            // ✅ Atualiza imediatamente a mensagem de boas-vindas com fullName
            const welcome = document.getElementById("welcomeMessage");
            if (welcome && result.user && result.user.fullName) {
              welcome.textContent = `Bem-vindo, ${result.user.fullName}!`;
            }
          }
        } catch (error) {
...
document.getElementById("closeModal").addEventListener("click", () => {
  document.getElementById("modalGerencia").classList.add("hidden");
});


document.getElementById("modalGerencia").addEventListener("click", (e) => {
